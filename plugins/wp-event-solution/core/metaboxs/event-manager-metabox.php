            ...
            <div class="etn-meta">
                <?php if ( !empty( $item['inputs'] ) &&  count($item['inputs'])>0 ) {
                    foreach( $item['inputs'] as $index => $item ) { 
                        $checked = in_array( $item, $value_arr )  ? 'checked' : '';
                        $item_key = strtolower(str_replace(' ','_',$item));
                    ?>
                    <input type ='checkbox' name='<?php echo esc_attr( $key ) . '['.$item_key.']'; ?>' id='<?php esc_attr_e($key.$index);?>' 
					class='etn_multi_checkbox' value="<?php esc_attr_e($item);?>" <?php esc_attr_e( $checked ) ;?>/>
