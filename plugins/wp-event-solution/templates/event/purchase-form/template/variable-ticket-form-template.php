...
<form method="post" class="etn-event-form-parent etn-ticket-variation"  data-etn_uid="<?php echo esc_html($unique_id); ?>" data-decimal-number-points="<?php echo esc_attr($price_decimal); ?>">
						...
						<div class="etn-single-ticket-item">
							...
							<div class="etn-ticket-price-body <?php echo esc_attr($stock_outClass) ?>">
								...
								<div class="ticket-price-item etn-quantity">
									...
									<input name="ticket_quantity[]" type="number"  class="etn_ticket_variation ticket_<?php echo intval($key); ?>" value="0" id="ticket-input_<?php echo intval($key); ?>" data-price="<?php echo number_format( (float) $value['etn_ticket_price'], $price_decimal, '.','' );?>" data-etn_min_ticket="<?php echo absint( $etn_min_ticket ); ?>" data-etn_max_ticket="<?php echo absint( $etn_max_ticket ); ?>" data-etn_current_stock="<?php echo absint( $etn_current_stock ); ?>" data-stock_out="<?php echo esc_attr__("All ticket has has been sold","eventin") ?>" data-cart_ticket_limit="<?php echo esc_attr__("You have already added 5 tickets. You can't purchase more than $etn_max_ticket tickets","eventin") ?>" data-stock_limit="<?php echo esc_attr__("Stock limit $etn_current_stock. You can purchase within $etn_current_stock.","eventin") ?>" data-qty_message="<?php echo esc_attr__("Total Ticket quantity must be greater than or equal ","eventin") . $etn_min_ticket . esc_attr__(" and less than or equal ","eventin") . $etn_max_ticket ; ?>" data-etn_cart_limit="<?php echo absint( $etn_cart_limit ); ?>" data-etn_cart_limit_message="<?php echo esc_attr__("You have already added $etn_cart_limit, Which is greater than maximum quantity $etn_max_ticket . You can add maximum $etn_max_ticket tickets. ","eventin"); ?>" />
	...
	if (!isset($event_options["etn_purchase_login_required"]) || (isset($event_options["etn_purchase_login_required"]) && is_user_logged_in())) {
		...
		?>
		<input name="submit" class="etn-btn etn-primary etn-add-to-cart-block disabled <?php echo esc_attr( absint( $single_event_id ) ); ?>" data-event_id="<?php echo esc_attr( absint( $single_event_id ) ); ?>" data-validation_checked="0" data-multivendor_active="<?php echo ( $multivendor_active ) ? '1' : '0'; ?>" type="submit" value="<?php $cart_button_text = apply_filters( 'etn_event_cart_button_text', esc_html__("Buy ticket", "eventin") ); echo esc_html( $cart_button_text ); ?>" />
				<?php
	} else {
		?>
		<small>
